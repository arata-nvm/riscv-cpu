...
class Core(suppressLog: Boolean) extends Module {
  ...
  val exe_inst_id = RegInit(~0.U(WORD_LEN.W))
  val mem_inst_id = RegInit(~0.U(WORD_LEN.W))
  val wb_inst_id = RegInit(~0.U(WORD_LEN.W))
  ...
  if_inst_id := Mux(if_stall_flg, if_inst_id, if_inst_id + 1.U)

  // IF/ID

  id_reg_pc := Mux(if_stall_flg, id_reg_pc, if_reg_pc)
  id_reg_inst := MuxCase(
    if_inst,
    Seq(
      (exe_br_flg || exe_jmp_flg) -> BUBBLE,
      if_stall_flg -> id_reg_inst
    )
  )
  ...
  if_stall_flg := id_rs1_data_hazard || id_rs2_data_hazard

  val id_inst =
    Mux(exe_br_flg || exe_jmp_flg || if_stall_flg, BUBBLE, id_reg_inst)
  id_inst_id := Mux(
    exe_br_flg || exe_jmp_flg || if_stall_flg,
    ~0.U(WORD_LEN.W),
    if_inst_id
  )
  ...
  val id_csr_addr =
    Mux(id_csr_cmd === CSR_E, CSR_ADDR_MCAUSE, id_inst(31, 20))
