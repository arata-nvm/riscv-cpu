...
RUN	cd riscv-gnu-toolchain && git submodule update --init --recursive --recommend-shallow --depth 1 riscv-binutils
RUN	cd riscv-gnu-toolchain && git submodule update --init --recursive --recommend-shallow --depth 1 riscv-gcc
RUN	cd riscv-gnu-toolchain && git submodule update --init --recursive riscv-newlib
RUN	cd riscv-gnu-toolchain && git submodule update --init --recursive --recommend-shallow --depth 1 riscv-gdb
RUN cd riscv-gnu-toolchain && mkdir build && cd build && ../configure --prefix=${RISCV} --enable-multilib && make -j$(nproc)
